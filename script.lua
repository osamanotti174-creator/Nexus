...
local Window = WindUI:CreateWindow({
    Title = "Nexus x dev 👨‍💻 | Free 💸",
    Icon = "rbxassetid://137877095538630",
    Author = "Block spin | PVP ⚔",
    Folder = "GotHubFolder",
...
MainTab:Toggle({
    ...
    Callback = function(v)
        for _, gui in ipairs(game:GetDescendants()) do
            if gui:IsA("BillboardGui") and gui.Name == "CharacterBillboardGui" then
                gui.Enabled = not v
            end
        end

        if v then
            print("CharacterBillboardGui OFF")
        else
            print("CharacterBillboardGui ON")
        end
    end
})
